...
3. **📋 テンプレートシステム**
   ...
   - 自動タイムスタンプ生成
   - 5つのキャラクターペルソナコメント
   - 冒頭15秒フックを問題提起→データ→価値約束の3文構造で強制
...
8. **🧠 インテリジェント・テキスト正規化**
   - **NEW**: 英単語を削除せず、VOICEVOXユーザー辞書で正しい発音を維持
   - 数値（カンマ対応）、日付、時刻、通貨（$など）の自然な日本語変換
   - 固有名詞の誤読を防ぐテクノロジー用語辞書（100語以上）
...
3. **🎙️ 自然な音声合成**
   ...
   - **NEW**: 高精度な日本語テキスト正規化（数値、日付、時刻、通貨$等の読みを自動変換）
...
6. **📝 SEO完全最適化 & スマート字幕レイアウト**
   - CTR最大化タイトル
   - タイムスタンプ付き説明文
   - 戦略的タグ配置
   - **NEW**: 左右マージン確保と自動改行による「見切れない」字幕デザイン
...
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │  台本 ──▶ テキスト正規化 ──▶ VOICEVOX Engine ──▶ WAV ──▶ MP3結合    │   │
│  │           (数値・通貨変換)    (ユーザー辞書適用)                        │   │
│  │           ┌─────────────────────────────────────────────────────┐   │   │
...
# v2.1修正: 無音カット処理を廃止し、Whisperによる強制アライメントで同期精度を向上させました。
# テキストハッシュによるキャッシュ管理で、音声と字幕の不一致（別物になる問題）も解消済みです。
```

**Q: 字幕が画面端で見切れる**
```python
# v2.1修正: 字幕の左右マージンを120pxに拡大し、1行の最大文字数を20文字に制限しました。
# 長文は自動的に適切に改行され、スマートフォンの画面でも見切れずに表示されます。
```
